import { Image, Network } from '@pulumi/docker';
...
export class RssBridgeKubeService extends ComponentResource {
  ...
  protected async initialize(args: RssBridgeInputs): Promise<any> {
    ...
    const rssBridgeConfigMap = new kube.core.v1.ConfigMap(
      'rss-bridge',
      {
        metadata: {
        ...
        data: {
          'whitelist.txt': readFileSync(
            join(__dirname, 'whitelist.txt'),
          ).toString(),
        },
      },
      {
    ...
    const rssBridgeDeployment = new kube.apps.v1.Deployment(
      'rss-bridge',
      {
        metadata: {
          namespace: namespace.id,
    ...
    const rssBridgeService = new kube.core.v1.Service(
      'rss-bridge',
      {
        metadata: {
          namespace: namespace.id,
    ...
    const rssBridgeScaledObject = new kube.apiextensions.CustomResource(
      'nginx-autoscale',
      {
    ...
    const rssBridgeTraefik = new kube.apiextensions.CustomResource(
      'nginx-proxy',
      {
        kind: 'IngressRoute',
