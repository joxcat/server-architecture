...
export class RegistryKubeService extends ComponentResource {
  ...
  protected async initialize(args: RegistryInputs): Promise<any> {
    ...
    const namespace = new kube.core.v1.Namespace('registry', {}, {
      parent: this,
    });

    const registryDeployment = new kube.apps.v1.Deployment('registry', {
      metadata: {
    ...
    const registryService = new kube.core.v1.Service('registry', {
      metadata: {
        namespace: namespace.id,
    ...
    const registryScaledObject = new kube.apiextensions.CustomResource('registry-autoscale', {
    ...
    const registryTraefik = new kube.apiextensions.CustomResource('registry-proxy', {
      kind: 'IngressRoute',
